...
with tab1:
    ...
    with col2:
        if analyze_button and (uploaded_files or directory_path):
            # Für dieses Beispiel verwenden wir nur den Verzeichnispfad
            if directory_path:
                with st.spinner('Analysiere Codebasis... Bitte warten.'):
                    try:
                        ...
                        # st.error() unterstützt unsafe_allow_html nicht direkt
                        st.markdown(f"""
                ...
                # st.warning() unterstützt unsafe_allow_html nicht direkt
                st.markdown("""
                <div style="background-color: #FEF3C7; color: #92400E; padding: 1rem; border-radius: 5px; margin-bottom: 1rem;">
                    <h4 style="margin-top: 0;">📋 Funktion für ZIP-Dateien</h4>
                    <p>Die Funktion für hochgeladene ZIP-Dateien ist noch nicht implementiert. Bitte verwenden Sie die Pfad-Eingabe.</p>
                </div>
                """, unsafe_allow_html=True)
        else:
            # st.info() unterstützt unsafe_allow_html nicht direkt
            st.markdown("""
            <div style="background-color: #F0F9FF; padding: 2rem; border-radius: 8px; margin-top: 3rem; text-align: center; border: 1px dashed #0EA5E9;">
                <img src="https://cdn-icons-png.flaticon.com/512/2541/2541983.png" width="80" style="margin-bottom: 1rem;">
                <h3 style="margin-top: 0; color: #0369A1;">Bereit zum Analysieren!</h3>
                <p style="color: #0284C7;">Bitte geben Sie einen Verzeichnispfad ein und klicken Sie auf 'Analysieren', um Ihre Codebasis zu evaluieren.</p>
            </div>
            """, unsafe_allow_html=True)
